...
func (p *ConnPool) _NewConn(ctx context.Context, pooled bool) (*Connection, error) {
...
func (p *ConnPool) newConn(ctx context.Context, pooled bool) (*Connection, error) {
	...
	endpoint, err := p.opt.Resolver(ctx)
	if err != nil {
...
func (p *ConnPool) tryDial() {
	for {
		...
		_, err := p.opt.Resolver(context.Background())
		if err != nil {
			p.setLastDialError(err)
...
func (p *ConnPool) popIdle() *Connection {
...
func (p *ConnPool) Remove(cn *Connection) {
...
func (p *ConnPool) reaper(frequency time.Duration) {
	...
	for range ticker.C {
		...
		if err != nil {
			zap.L().Error("ReapStaleConns failed", zap.Error(err))
			continue
		}
	}
}
